...
def oneNight(data,day,syms,cols,suf,add):
    ...
    sumPlots(data,syms,cols,'reltime','rmsmm','Spot Motion RMS vs Time ('+add+')',"Time (hrs)",'Spot Motion RMS (mm)',ylim,'over_'+suf,day)
    
    ylim=[1,5]
    sumPlots(data,syms,cols,'reltime','fx','FWHM(x) vs Time',"Time (hrs)",'FWHM(x)',ylim,'over_'+suf,day)
    sumPlots(data,syms,cols,'reltime','fy','FWHM(y)',"Time (hrs)",'FWHM(y)',ylim,'over_'+suf,day)
    
    ylim=[0,0.01]
    
    #sumPlots(data,syms,cols,'id','rmsmm','Spot Motion RMS vs Frame ID',"Frame ID",'Spot Motion RMS (Pixels)',ylim,'over_'+suf,day)
    #
    #ylim=[1,5]
    #sumPlots(data,syms,cols,'id','fx','FWHM(x) vs Frame ID',"Frame ID",'FWHM(x)',ylim,'over_'+suf,day)
    #sumPlots(data,syms,cols,'id','fy','FWHM(y)',"Frame ID",'FWHM(y)',ylim,'over_'+suf,day)
    #
    #
    ##rms, fwhms vs elevation
    #ylim=[0,0.01]
    #sumPlots(data,syms,cols,'el','rmsmm','Spot Motion RMS vs Elevation',"Elevation",'Spot Motion RMS (mm)',ylim,'over_'+suf,day)
    #ylim=[1,5]
    #sumPlots(data,syms,cols,'el','fx','FWHM(x) vs Elevation',"Elevation",'FWHM(s)',ylim,'over_'+suf,day)
    #sumPlots(data,syms,cols,'el','fy','FWHM(y) vs Elevation',"Elevation",'FWHM(y)',ylim,'over_'+suf,day)
    #
    ##rms, fwhms vs exposure time
    #ylim=[0,0.01]
    #sumPlots(data,syms,cols,'t','rmsmm','Spot Motion RMS vs Exposure Time',"Exposure Time",'Spot Motion RMS (mm)',ylim,'over_'+suf,day)
    #ylim=[1,5]
    #sumPlots(data,syms,cols,'t','fx','FWHM(x) vs Exposure Time',"Exposure Time",'FWHM(s)',ylim,'over_'+suf,day)
    #sumPlots(data,syms,cols,'t','fy','FWHM(y) vs Exposure Time',"Exposure Time",'FWHM(y)',ylim,'over_'+suf,day)
    #
    ##fx vs rms and each other
    #
    #ylim=[0,0.01]
    #sumPlots(data,syms,cols,'fx','rmsmm','Spot Motion RMS vs FWHM(x)',"FWHM(x)",'Spot Motion RMS (mm)',ylim,'over_'+suf,day)
    #sumPlots(data,syms,cols,'fy','rmsmm','Spot Motion RMS vs FWHM(y)',"FWHM(y)",'Spot Motion RMS (mm)',ylim,'over_'+suf,day)
    #ylim=[1,5]
    #
    #sumPlots(data,syms,cols,'fx','fy','FWHM(y) vs FWHM(x)',"FWHM(x)",'FWHM(y)',ylim,'over_'+suf,day)
    
    #ylim=None
    #sumPlots(data,syms,cols,'reltime','domhum','Dome Humidity vs time','time',"Dome Humidity",ylim,'over_'+suf,day)
    #sumPlots(data,syms,cols,'reltime','domwnd','Dome Wind Speed vs time',"time,","Dome Wind Speed",ylim,'over_'+suf,day)
    #sumPlots(data,syms,cols,'reltime','domprs','Dome Pressure vs time',"time","Dome Pressure",ylim,'over_'+suf,day)
    #sumPlots(data,syms,cols,'reltime','domtmp','Dome Temperature vs time',"time","Dome Temperature",ylim,'over_'+suf,day)
    #sumPlots(data,syms,cols,'reltime','outtmp','Outside Temperature vs time',"time","Outside Temperature",ylim,'over_'+suf,day)
    #sumPlots(data,syms,cols,'reltime','mcm1t','Mirror Temperature vs time',"time","Mirror Temperature",ylim,'over_'+suf,day)
...
def readSpread(fname,source):

    data = np.genfromtxt(fname, delimiter=None, names=True,dtype=None)
    data = massageData(data,source)
    return data


    
    
def runSet(day,source,add):
    ...
    infile=source+"_rmsC.dat"
    suf=source
    data=readSpread(infile,source)
    oneNight(data,day,syms,cols,suf,add)
...
if __name__ == "__main__":    
    ...
    #runSet(5)
    runSet(-1,"l","MCS")
    runSet(-1,"w","Sextractor Windowed")
    runSet(-1,"p","PSF Fitting")
    runSet(-1,"t","Moment")
