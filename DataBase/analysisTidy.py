...
def readFPS(frameID,loadPref):
    ...
    ff=np.load(loadPref+"/fpsData_"+str(frameID)+".npz")

    centroids=ff['arr_0'][0]
    za=ff['arr_0'][1]
    inr=ff['arr_0'][2]
    rotCent=ff['arr_0'][3]
    offset=ff['arr_0'][4]
    fiducials=ff['arr_0'][5]
    scienceFibres=ff['arr_0'][6]
    fiducials=ff['arr_0'][7]
    fibres=ff['arr_0'][8]
    allPos=ff['arr_0'][9]
    fidPos=ff['arr_0'][10]
    sciPos=ff['arr_0'][11]
    ...
    return centroids,za,inr,rotCent,offset,fiducials,scienceFibres,fiducials,fibres,allPos,fidPos,sciPos,cc,ccTrans,fCentroid,trans,xd,yd,sx,sy,rot,xx1,yy1,aCent,dx,dy
...
def readFibres(frameID,loadPref):
    ...
    vals=np.load(loadPref+"/ndump_"+str(frameID)+".npy")
    ...
    xPix=convertArray(xPix)
    yPix=convertArray(yPix)
    fx=convertArray(fx)
    fy=convertArray(fy)
    peak=convertArray(peak)
    xy=convertArray(xy)
    qual=convertArray(qual)
    xNom=convertArray(xNom)
    yNom=convertArray(yNom)
    dx=convertArray(dx)
    dy=convertArray(dy)
...
def readTrans(frameID,loadPref):
    ...
    afCoeff=np.load(loadPref+"/ntrans_"+str(frameID)+".npy")
...
def readPhys(frameID,loadPref):
    ...
    phys["t"]=np.float(ff[0])    
    phys["el"]=np.float(90-np.float(ff[1]))    
...
def readParms(frameID,loadPref):
    
    ff=np.load(loadPref+"/nparm_"+str(frameID)+".npy")

    parms={}

    parms['findThresh']=np.int(ff[0])
    parms['centThresh']=np.int(ff[1])
    parms['boxFind']=np.int(ff[2])
    parms['boxCent']=np.int(ff[3])
    parms['nmin']=np.int(ff[4])
    parms['nmax']=np.int(ff[5])
    parms['maxIt']=np.int(ff[6])
